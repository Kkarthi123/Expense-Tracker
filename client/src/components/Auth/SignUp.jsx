...
const SignUp = () => {
  ...
  return (
    <div>
      <div className="shadow-md rounded-lg px-5 py-8 pb-7 w-[400px] bg-white">
        <div className="mb-7 text-center">
          <span className="text-xl font-bold">Sign Up with Expense Tracker</span>
        </div>
        <div>
          <OauthBtn textName="Sign Up with Google"/>
        </div>
        <div className="text-center text-sm my-7 relative">
          <span className="absolute border border-gray-200 w-full left-0 top-[9px]"></span>
          <span className="bg-white relative z-[1] inline-block px-[5px] text-gray-500">Or Sign Up with Email</span>
        </div>
        <form onSubmit={(e) => signUpFormSubmit(e)}>
          <div className="flex flex-col mb-5">
            <label htmlFor="name" className="block text-[15px] font-medium text-gray-500">User Name</label>
            <input type="text" id="name" value={userName} onChange={(e) => setUserName(e.target.value)} className="bg-gray-50 border border-gray-300 text-gray-600 text-sm rounded-lg block w-full p-2.5 outline-none focus-within:border-gray-400" placeholder="Enter User Name" required />
          </div>
          <div className="flex flex-col mb-5">
        ...
        <div className='mt-3 text-[13px]'>
          <span>Already have an account? <Link to="/auth/login" className='underline text-blue-500 hover:text-blue-600'>Sign In.</Link></span>
        </div>
      </div>
    </div>
  )
